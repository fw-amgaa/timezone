 ...
 *
 * Set the initial password for a user after OTP verification.
 * Used for first-time registration flow.
 */
  ...
  try {
    ...
    if (user.passwordHash) {
      return NextResponse.json(
        {
          success: false,
          error: "Password already set. Use login or password reset instead.",
        },
        { status: 400 }
      );
    }
    ...
    // Update user with password and mark phone as verified
    await db
      .update(users)
      .set({
        passwordHash,
